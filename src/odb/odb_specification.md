...
Transactions in react should be created like this
```
const transaction = useOdbTransaction(...);

return <OdbTransaction transaction={transaction}>
...
</OdbTransaction>;
```
...
Ui/react features:
- Context to store Odb and OdbTransaction
	- Transaction in react has to be created like this
		- ```
		  ```
...
Main usage tests:
- Initialization of Odb in react app
- Creating and passing context and hooks, and that UI components actually work with all these
- Including Odb
- Including OdbTransaction
...
This specification is always available in src/odb/odb_specification.md.
Please create review and update src/odb/odb_llm_plans.md from time to time so you could continue later on.
Please implement odb...